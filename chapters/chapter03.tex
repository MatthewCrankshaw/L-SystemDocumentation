...
\lettrine[lines=3]{T}{}here are two major parts necessary to procedurally generate plant-life using L-systems. These are the L-system rewriter and the L-system interpreter. The L-system rewriters purpose is to take a defined L-system which complies with a partacular grammar, and carry out the rewriting process, eventually giving a resultant string. The interpreter runs through each character or instruction within the resulting string  and interprets its meaning. The resultant string acts as a set of instructions which are carried out by a turtle graphics interpreter to eventually represent a model of a plant on the screen. This chapter will focus on the first major part of this process, the rewriting system. 

Due to the complexity of the L-system grammar, having a computer properly understand the L-system language in order to carry out rewriting can be a difficult task. This chapter will describe a technique of processing the L-system to simplify the rewriting process, similar to how computer languages are compiled before executing. Using a \say{compiler} to process an L-system, the L-system \say{program} can be passed through a three major stages of the rewriting process. Firstly is lexical analysis, then parsing and finally string rewriting. Each component of the L-system will be analysed and parsed into the appropriate computational structures. If the syntax is valid and the parser is successful, the structures created will be used to rewrite the L-system. If the L-system provided does not match the context-free grammar definition, an appropriate error message can be displayed, similar to that of a compilation error of a computer language.

For simple D0L-system seen in section \ref{DOL-system example}, creating a rewriter is relatively  easy. Each rewritable symbol is only made up of a single character, and the D0L-system is deterministic, so we know that there is no randomess when determining the matching rule. This is just a case of storing the starting string, and then iterating through this string one character at a time and when a symbol matches that in a rule, replace that character with the string provided by the rule. This can be implemented relatively simply, however, to implement the parametric 0L-system, where each instruction is a module with potentually multiple parameters and each parameter could be a mathematical expression, the rewriting system will have to better understand what each part of the L-system is specifying based on the grammar and its context in the L-system. The lexical analyser and parser can be used for this purpose in order to classify each part of the L-system and prepare it for rewriting.
